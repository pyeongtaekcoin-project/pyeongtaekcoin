#!/usr/bin/env bash
# Copyright (c) 2016 The Pyeongtaekcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   pyeongtaekcoincore.org and pyeongtaekcoin.org and compares them.
...
WORKINGDIR="/tmp/pyeongtaekcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://pyeongtaekcoincore.org"
HOST2="https://pyeongtaekcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="pyeongtaekcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'pyeongtaekcoin-',
...
if [ $? -ne 0 ]; then
   echo "pyeongtaekcoin.org failed to provide signature file, but pyeongtaekcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "pyeongtaekcoin.org and pyeongtaekcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Pyeongtaekcoin Core binary release signing key installed?"
   fi
