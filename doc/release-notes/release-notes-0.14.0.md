Pyeongtaekcoin Core version 0.14.0 is now available from:

  <https://pyeongtaekcoin.org/bin/pyeongtaekcoin-core-0.14.0/>
...
Pyeongtaekcoin Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/pyeongtaekcoin/pyeongtaekcoin/blob/master/doc/zmq.md) for
...
The Alert System was [disabled and deprecated](https://pyeongtaekcoin.org/en/alert/2016-11-01-alert-retirement) in Pyeongtaekcoin Core 0.12.1 and removed in 0.13.0. 
...
- #8722 `bb843ad` pyeongtaekcoin-cli: More detailed error reporting (laanwj)
...
- #7562 `1eef038` Bump transaction version default to 2 (ptcdrak)
...
- #8608 `7e9ab95` Install manpages via make install, also add some autogenerated manpages (nomnomptc)
...
- #8568 `078900d` new var `DIST_CONTRIB` adds useful things for packagers from contrib (nomnomptc)
...
- #8830 `ef0801b` test: Add option to run pyeongtaekcoin-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to pyeongtaekcoin-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to pyeongtaekcoin-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and pyeongtaekcoin-util-test.py (jnewbery)
...
- #9206 `e662d28` Make test constant consistent with consensus.h (ptcdrak)
...
- #9691 `fc67cd2` Init ECC context for `test_pyeongtaekcoin_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change pyeongtaekcoin address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` pyeongtaekcoind: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow pyeongtaekcoin-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` pyeongtaekcoin-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to pyeongtaekcoin-tx (jnewbery)
