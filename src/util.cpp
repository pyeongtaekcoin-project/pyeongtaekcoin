...
const char * const PYEONGTAEKCOIN_CONF_FILENAME = "pyeongtaekcoin.conf";
const char * const PYEONGTAEKCOIN_PID_FILENAME = "pyeongtaekcoin.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "pyeongtaekcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Pyeongtaekcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Pyeongtaekcoin
    // Mac: ~/Library/Application Support/Pyeongtaekcoin
    // Unix: ~/.pyeongtaekcoin
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", PYEONGTAEKCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", PYEONGTAEKCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Pyeongtaekcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Pyeongtaekcoin Core") == std::string::npos) {
        std::string strYear = strPrefix;
        strYear.replace(strYear.find("2011"), sizeof("2011")-1, "2009");
        strCopyrightHolders += "\n" + strYear + "The Pyeongtaekcoin Core developers";
    }
