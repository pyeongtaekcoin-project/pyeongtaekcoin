...
const int PYEONGTAEKCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString PYEONGTAEKCOIN_IPC_PREFIX("pyeongtaekcoin:");
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the pyeongtaekcoin: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(PYEONGTAEKCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // pyeongtaekcoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parsePyeongtaekcoinURI(arg, &r) && !r.address.isEmpty())
            {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(PYEONGTAEKCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(PYEONGTAEKCOIN_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click pyeongtaekcoin: links
...
//
// OSX-specific way of handling pyeongtaekcoin: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in pyeongtaekcoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    else if (s.startsWith(PYEONGTAEKCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // pyeongtaekcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parsePyeongtaekcoinURI(s, &recipient))
            {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom pyeongtaekcoin addresses are not supported
        ...
        // Pyeongtaekcoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(PyeongtaekcoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
