...
ClientModel is used by the main application `pyeongtaekcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/pyeongtaekcoin/pyeongtaekcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a pyeongtaekcoin:... link from a browser).
...
* UI elements like PyeongtaekcoinAmountField, which inherit from QWidget.
* `pyeongtaekcoinstrings.cpp`: automatically generated
* `pyeongtaekcoinunits.(h/cpp)`: PTC / mPTC / etc handling
* `callback.h`
...
* don't change `local/pyeongtaekcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
