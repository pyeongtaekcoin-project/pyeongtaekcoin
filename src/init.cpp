 ...
 * between pyeongtaekcoind, and pyeongtaekcoin-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", PYEONGTAEKCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", PYEONGTAEKCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
/** Sanity checks
 *  Ensure that Pyeongtaekcoin is running in a usable environment with all
...
{
    // Make sure only a single Pyeongtaekcoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", PYEONGTAEKCOIN_CONF_FILENAME)).string());
